...
import numpy as np
from src.zoo.rtdetr.video_swintransformer.video_swintransformer_utils import trunc_normal_
...
class WindowAttention3D(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        super().__init__()
        self.dim = dim                  # 256
        self.window_size = window_size  # Wd, Wh, Ww   5,20,20
        self.num_heads = num_heads      # 256/64  4
        head_dim = dim // num_heads     # 256/4   64
...
class SwinTransformerLayer3D(nn.Module):
    ...
    def forward(self, x):
        ...
        mask_matrix = create_mask(d, h, w, window_size, shift_size, x.device,
                                  self.attn.qkv.weight.dtype)  # [nW, Mh*Mw, Mh*Mw]
...
class SwinTransformerBlock3D(nn.Module):
    def __init__(self, c1, num_frames=5, num_layers=2):
        super().__init__()
        num_heads = c1 // 64
        window_size = 10    # default 8
        self.num_frames = num_frames
        self.window_size = (num_frames, window_size,
                            window_size)  # (num_frames, window_size, window_size) # (math.ceil(num_frames/2), window_size, window_size)
        self.shift_size = (0, window_size // 2,
                           window_size // 2)  # (0, window_size // 2, window_size // 2) #(num_frames//2, window_size // 2, window_size // 2)
        self.tr = nn.Sequential(*(SwinTransformerLayer3D(c1, num_heads=num_heads, window_size=self.window_size,
                                                         shift_size=(0, 0, 0) if (i % 2 == 0) else self.shift_size) for i in range(num_layers)))
